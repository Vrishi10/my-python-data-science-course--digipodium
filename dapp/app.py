...
def load_dataset():
    ...
    df.rename(columns={'OdName':'Country','AreaName':'Continent','RegName':'Region','DevName':'Status'},
              inplace=True)
...
st.metric("Total Immigrants", f'{total_immigrants}' )
...
#graphs under plotly heading

#country_df = df.loc[sel_country, years]
#fig = px.area(y=country_df, x=country_df.index)
#st.plotly_chart(fig)

#country_df = df.loc[sel_country, years]
#fig = px.bar(y=country_df, x=country_df.index)
#st.plotly_chart(fig)
